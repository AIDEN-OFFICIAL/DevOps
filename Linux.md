...
| `rm` | **R**e**m**oves a file. | `rm old_file.txt` |
...
|`shread`|to never let someone see a file we can shread it|`shread filename.txt`|
...
|`userdel`|to delete a user|`sudo userdel newname`doesn't delete home directory.`-r` for deleteing home dir also|
|`su`|switching user |`su another_username`|
|`exit`|exit from one user to default user|`exit`|
|`wc`|gives the count of the content in a file like the lines, words and characters|`wc filename` `wc filename -L` length of the longest line|
...
|`whatis`|just like man with les info , or a shortest description of a command|`whatis grep` `whatis ls` `whatis finger`|
...
|`zip`|to create zipfiles|`zip new_zip_name.zip old_name.txt`|
...
|`ifconfig`|to know the ip address and whole configurations of the network connections |`ifconfig` `ifconfig eth0 down` the eth0 will be disconnected `ifconfig eth0 up`|
